...
## Project and Read-Only Precedence

This optional Orchestrator workflow is subordinate to repository instructions,
tracker policy, and explicit user confirmation. It is unavailable to read-only
specialists. `.slim/worktrees.json` is optional local metadata, never a task
tracker: reuse existing state or keep it ephemeral when project policy does not
permit it. Do not modify ignore files automatically; report exact entries and
obtain authorization first. Use platform-native Git commands and resolved paths
rather than assuming Bash or POSIX syntax.
