...
Read the project's package manifest, lockfile, component library, and existing
frontend guidance before applying patterns. Use only APIs supported by the
installed Vue version; newer reactive-destructuring, SSR, or performance claims
are conditional. Prefer the project's existing components, styling system, and
test setup. Do not introduce Nuxt UI, Reka UI, VueUse, JSX, or other dependencies
unless they are already installed and appropriate for the task.
...
For styled UI, headless components, or composables, use existing project
dependencies and documented conventions before adding or loading another stack.
