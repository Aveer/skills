...
These are optional design patterns, not universal mandates. First read the
project's UI guidance, tokens, component library, accessibility rules, density,
and motion policy. Preserve the existing technology stack; do not introduce
React, Tailwind, Framer Motion, or another dependency from examples. Treat exact
values, shadows-over-borders, staggering, and hit-area sizes as context-dependent
starting points, then validate them against the product and platform.
