...
Repository `AGENTS.md`, nested guidance, codemaps, tracker policy, and current
documentation override this skill. For read-only reconnaissance, inspect and
report only: do not create state, migrate files, generate templates, edit
`AGENTS.md`, update hashes, or delegate writing. `.slim/codemap.json` is cache
state, never a task tracker. Generation or updates require explicit user and
project authorization; use resolved platform-native paths rather than assuming
Bash, `~/.config`, or POSIX syntax.
