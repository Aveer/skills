...
The commands below are POSIX illustrations, not mandatory syntax. On Windows,
use a unique directory under `$env:TEMP`, PowerShell or Python to write files,
quoted `Path` values, and a Windows-compatible timeout. Detect the available
package runtime (`npm`, `pnpm`, or `bun`) rather than assuming Bun. Preserve the
contract: pack the exact artifact, install it into an isolated throwaway app,
inspect the resolved plugin origin, run the smoke, and record limitations.

Never use a host provider secret in an isolated config. Use documented
environment/file interpolation or clearly label a host-provider smoke as less
isolated.

## POSIX Pack Candidate Example
