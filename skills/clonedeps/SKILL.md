...
## Project Precedence and Read-Only Boundary

Repository dependency, storage, tracker, and publication policies override this
workflow. Inspection is read-only. Cloning, fetching, moving, deleting,
updating manifests, or editing ignore files and `AGENTS.md` require explicit
user authorization. `.slim/clonedeps.json` is dependency-source metadata, never
a task tracker; reuse existing entries rather than creating parallel state. Use
platform-native path handling and never run install, build, or test commands
inside a cloned dependency unless explicitly authorized.
